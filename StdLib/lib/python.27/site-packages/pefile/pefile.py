...
class PE(object):
    ...
    def close(self):
        # EfiPy if ( self.__from_file is True and hasattr(self, '__data__') and
        # EfiPy     ((isinstance(mmap.mmap, type) and isinstance(self.__data__, mmap.mmap)) or
        # EfiPy    'mmap.mmap' in repr(type(self.__data__))) ):
        # EfiPy         self.__data__.close()
        if self.__from_file is True and self.fname is not None:
    ...
    def __parse__(self, fname, data, fast_load):
        ...
        if fname:
            ...
            try:
                ...
                # EfiPy if hasattr(mmap, 'MAP_PRIVATE'):
                # EfiPy     # Unix
                # EfiPy     self.__data__ = mmap.mmap(self.fileno, 0, mmap.MAP_PRIVATE)
                # EfiPy else:
                # EfiPy     # Windows
                # EfiPy     self.__data__ = mmap.mmap(self.fileno, 0, access=mmap.ACCESS_READ)
    ...
    def parse_rich_header(self):
        ...
        try:
            ...
            # print 'rich_data:%s 0x%X' % (type (rich_data), len (rich_data))
    ...
    def parse_relocations(self, data_rva, rva, size):
        ...
        entries = []
        offsets_and_type = []
        # EfiPy for idx in range( old_div(len(data), 2) ):
    ...
    def parse_debug_directory(self, rva, size):
        ...
        debug = []
        # EfiPy for idx in range(old_div(size,dbg_size)):
