...
OutStrF (ConOutP,
  u"gRT.GetVariable(Name: \"%s\" (Status:0x%016X)\r\n" % (VariableName, Status))
...
if Status == EfiPy.EFI_BUFFER_TOO_SMALL:
  ...
  OutStrF (ConOutP,
    u"gRT.GetVariable(Name: \"%s\" (Size:%d)\r\n" % (VariableName, DataSize.value))
  ...
  DataBuffer    = EfiPy.create_string_buffer ("AbCdE", 5)
  Attributes    = EfiPy.UINT32(
  ...
  OutStrF (ConOutP,
    u"gRT.SetVariable(Name: \"%s\" (Status:0x%016X)\r\n" % (VariableName, Status))
